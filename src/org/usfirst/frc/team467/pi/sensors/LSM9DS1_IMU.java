 ...
 */
public class LSM9DS1_IMU {

	private static final double MEASURES_PER_DEGREE = 1.0;

	private static final long kCalibrationSampleTime = 5000;
	...
	public void calibrate() {
		...
		try {
			Thread.sleep(kCalibrationSampleTime);
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
	...
	private void acquire() {
		synchronized (this) {
			m_last_sample_time = System.currentTimeMillis();
		}
		while (!m_freed.get()) {

			double sample_time = System.currentTimeMillis();
			double dt;
			synchronized (this) {
	        ...
	        driver.pollIMU();
	        double gyro_x = driver.getGyrPitchX();
			double gyro_y = driver.getGyrRollY();
			double gyro_z = driver.getGyrYawZ();
			double accel_x = driver.getAccX();
			double accel_y = driver.getAccY();
			double accel_z = driver.getAccZ();

	        driver.pollMagnetometer();
	        double mag_x = driver.getMagX();
			double mag_y = driver.getMagY();
			double mag_z = driver.getMagZ();

			driver.pollThermometer();
			double temp = driver.getTemperature();
